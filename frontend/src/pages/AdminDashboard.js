...
const AdminDashboard = () => {
  ...
  const ladderMatches = matches.filter(m => m.bracketType === 'ladder').sort((a,b)=>a.matchNumber-b.matchNumber);

  const [tForm, setTForm] = useState({
    name: '', sport: 'cricket', maxTeams: 8, playersPerTeam: 11,
    venue: '', startDate: '', endDate: '', overs: 20, description: '',
    registrationDeadline: '', prizeInfo: '', status: 'registration_open', tournamentFormat: 'Single Knockout',
  });
  ...
  //   try { 
  //     await matchAPI.generate(tId); showMsg('⚡ Fixture generated!'); await fetchMatchesForTournament(tId); }
  //   catch (err) { setError(err.response?.data?.message || 'Error generating fixture'); }
  // };
  const handleGenerateFixture = async (tId) => {
    const tournament = tournaments.find(t => t._id === tId);
    const format = tournament?.tournamentFormat || 'Single Knockout';

    if (!window.confirm(`Generate ${format} fixture for "${tournament?.name}"? Existing matches will be deleted.`)) return;

    try {
      await matchAPI.generate(tId, { format }); // pass format to backend
      showMsg(`⚡ ${format} fixture generated!`);
            ...
            <form onSubmit={handleCreateTournament}>
              ...
              <div className="form-group">
                ...
                >
                  ...
                  <option value="Repechage Tournament">Repechage Tournament</option>
                  <option value="Pyramid Tournament">Pyramid Tournament</option>
                </select>
              </div>

              <div className="form-group">
                {/* Leave empty to maintain grid alignment */}
              </div>
              {/* </div> */}
              <div className="grid-2">
                <div className="form-group">
