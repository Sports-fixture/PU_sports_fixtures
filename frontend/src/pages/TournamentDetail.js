...
import LadderFixtureBuilder from '../pages/LadderFixtureBuilder';
...
const TournamentDetail = () => {
  ...
  return (
    <div className="page-container page-wrapper fade-in">
      ...
      {activeTab === 'fixtures' && (
        <div className="fade-in">
          {isAdmin && tournament.tournamentFormat === 'Ladder Tournament' && matches.length === 0 && (
            <LadderFixtureBuilder tournamentId={id} approvedTeams={approvedTeams} onGenerated={fetchData} />
          )}
          ...
          {matches.length === 0 ? (
            ...
            <>
              ...
              {tournament.tournamentFormat === 'Ladder Tournament' ? (
                ladderMatches.filter(m => m.status !== 'live').length > 0 && (
                  <div style={{ marginBottom: 28 }}>
                    <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'var(--royal)', marginBottom: 12 }}>🪜 Ladder Round</h3>
                    {ladderMatches.filter(m => m.status !== 'live').map(m => <ScheduledMatchCard key={m._id} match={m} color="var(--royal)" />)}
                  </div>
                )
              ) : (
                [
