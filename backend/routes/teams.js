...
router.post("/register", auth, async (req, res) => {
  try {
    ...
    if (existingTeam) {
      ...
      if (existingTeam.status === "rejected") {
        await Team.findByIdAndDelete(existingTeam._id); // remove old rejected entry
      } else {
    ...
    res
      .status(201)
      .json({
        message: "Team registered successfully, pending approval",
        team,
      });
  } catch (err) {
    res.status(500).json({ message: err.message });
...
router.get("/my/teams", auth, async (req, res) => {
  try {
    const teams = await Team.find({ registeredBy: req.user._id }).populate(
      "tournament",
      "name sport status",
    );
    res.json(teams);
